...
// OfN - Migration print statement big progs
void PR_PrintStatement7(int index)
{
	int		i, op, a, b, c;
...
void PR_ExecuteProgram (func_t fnum)
{
	...
	dstatement7_t *st7 = NULL; // OfN - Migration
	dfunction_t *f, *newf;
	int runaway;
	int i;
	edict_t *ed;
...
#define ST(field)	(pr_statements? st->field : st7->field)
		...
		*/
		...
		{
			if (prozacmod_loaded)
				PR_PrintStatement7(s);
			else
				PR_PrintStatement(st);
		}

		//switch (st->op) MIGRATION
		switch (ST(op))
		{
		case OP_ADD_F:
		...
		default:
			...
			PR_RunError("Bad opcode %i", ST(op)); // MIGRATION
		}
	}
