...
// OfN
cvar_t sv_warnmodels = { "sv_warnmodels","0" };
// OfN End
	...
	if (!sv_client->rip_vip)
	{
		if (sv_client->state == cs_preconnected)
		{
			...
			if (sv_client->vip && sv_client->spectator)
				...
				//Sys_Printf ("Spectator %s connected\n", sv_client->name);// August 2020
				Sys_Printf("Spectator %s connected (%s)\n", sv_client->name, NET_AdrToString(net_from));// August 2020
			else
				//Sys_Printf ("Client %s connected\n", sv_client->name); // August 2020
				Sys_Printf("Client %s connected (%s)\n", sv_client->name, NET_AdrToString(net_from));// August 2020
...
static void Cmd_Begin_f (void)
{
	...
	SV_RetrieveClientVersion(sv_client);
	...
	if (!sv_client->spectator && sv_warnmodels.value)
	{
...
void SV_CompleteDownoload(void)
{
	...
	if (sv_client->is_special_download)
	{
		...
		Con_Printf("Special map file finished downloading, you must reload/reconnect for it to take effect on the map...\n");
	}
	else
		Con_Printf((char *)Q_redtext(download_completed));
...
static void SV_NextUpload (void)
{
	...
	//Sys_Printf("-"); // Commented in October 2020
...
#define CLIENT_DOWNLOAD_RELATIVE_BASE FS_BASE
	...
	if (sv_client->state < cs_spawned) // Only if not spawned
	{
		if (strcmp(Info_Get(&sv_client->_userinfoshort_ctx_, "team"), "äìéî") != 0) // Only if it isn't already set
		{
			...
			MSG_WriteString(&sv.reliable_datagram, Info_Get(&sv_client->_userinfoshort_ctx_, "team"));
		}
	}

	// Flag download as special if any extra file extension type is gonna download //
	int filenamelen = strlen(name);
	
	sv_client->is_special_download = false;

	char tempstr[6];
	if (filenamelen > 4)
	{
		for (i = filenamelen - 4, x = 0; i < filenamelen; i++, x++)
		{
			tempstr[x] = name[i];
		}
		tempstr[4] = '\0';

		if (!strcasecmp(tempstr, ".lit") || !strcasecmp(tempstr, ".sky") || !strcasecmp(tempstr, ".hrt"))
			sv_client->is_special_download = true;
	}
	// OfN 2020 - End
...
static void SV_Say (qbool team)
{
	...
	// unfake if requested.
	//if (!team && sv_unfake.value) // original code before February 2021
	if ((!team && sv_unfake.value) || sv_unfake.value == 2)
	{
		char *ch;
	...
	// OfN - /me support
	if (sv_client->spectator == 0 && (!strncasecmp(p, "/me ", 4) || !strncasecmp(p, "\"/me ", 5)))
	{
		is_me = true;
		me_offset = 4;
	}
	...
	// remove surrounding " if any. OfN - Added the "+ me_offset"s
	if (p[0] == '"' && (j = (int)strlen(p)) > 2 && p[j-1] == '"')
	{
		// form text[].
		snprintf(text, sizeof(text), "%s", p + 1  + me_offset  ); // skip opening " and copy rest text including closing ".
	...
	for (j = 0, client = svs.clients; j < MAX_CLIENTS; j++, client++)
	{
		...
		if (team)
		{
			...
			else
			{
				...
				// Original code - else if (strcmp(sv_client->team, client->team))
					// Original code - continue; // on different teams
				else if (!team_no_offset || !Teammate_pf) // Teammate progs feature not mod supported?
				{
					if (strcmp(sv_client->team, client->team)) // Do a comparison of standard "team"s then...
						continue; // on different teams
				}
				else // Support for "Teammate" progs function on the mod
				{
					// Get the team_no's for the 2 client edicts
					eval_t* team1; eval_t* team2;
					team1 = (eval_t *)((char *)&client->edict->v + team_no_offset);
					team2 = (eval_t *)((char *)&sv_client->edict->v + team_no_offset);
					
					// Assign parameters for progs usage
					G_FLOAT(OFS_PARM0) = (float)team1->_float;
					G_FLOAT(OFS_PARM1) = (float)team2->_float;

					// Call the progs function to determine if a teammate, including diff teams, which is the point
					PR_ExecuteProgram(Teammate_pf);

					// Check returning value from the progs function
					if (G_FLOAT(OFS_RETURN) == (float)0) // did the progs function return a 0 (FALSE)
						continue; // yep, so just skip this target client for the team message
				}
			}
		}
		else
		...
		if (is_me)
		{
			SV_ClientPrintf2(client, PRINT_HIGH, "%s", text);
			ClientReliableWrite_Begin(client, svc_stufftext, 2 + strlen("play misc/talk.wav\n"));
			ClientReliableWrite_String(client, "play misc/talk.wav\n");
		}
		else // Original code follows
...
static void Cmd_Upload_f (void)
{
	...
	{
		if (!strstr(sv_client->uploadfn, "/qwprozac.dat")) // October 2020 Allow progs overwrite!
		{
			// Original code - Always this
...
static void Cmd_Join_f (void)
{
	...
	// OfN - Get the client type/version and set them on corresponding c/quakec shared fields.
	SV_RetrieveClientVersion(sv_client); // Check if this works! (?) July 2020
	...
	{// added
	 // OfN - Client commands handled on progs
		if (ClientCommand)
		{
			...
			{
				if (j < tmp)
					PR1_SetString(&((int *)pr_globals)[OFS_PARM1 + j * 3],Cmd_Argv(j));
				else
		...
		Con_Printf("Bad user command: %s\n", Cmd_Argv(0)); // <-- Only original code here
	}// added
