...
// OfN
cvar_t sv_pinsfile = { "sv_pinsfile","pinmsgs.pm" };
cvar_t sv_showheartbeats = { "sv_showheartbeats","0" };
startup_timedata_t startup_time;
...
void SV_DropClient (client_t *drop)
{
	...
	if (drop->spectator)
		Con_Printf ("Spectator %s removed (%s)\n",drop->name, NET_AdrToString(drop->realip));//August 2020
	else
		Con_Printf ("Client %s removed (%s)\n",drop->name, NET_AdrToString(drop->realip));//August 2020
...
void FixMaxClientsCvars(void)
{
	...
	/* BUG? OR INTENDED, I COMMENTED OUT THIS BULLSHIT (REQUETEPUTA maxclients)
...
//
	...
	// OfN - Display pinned messages to him/her...
	SV_DisplayPinmsgs(newcl);
}
...
qbool rcon_from_user = false; // is the current command an rcon from user
extern char* Name_of_sender(void);
...
static void SV_ConnectionlessPacket (void)
{
	...
	if (!strcmp(c, "ping") || ( c[0] == A2A_PING && (c[1] == 0 || c[1] == '\n')) )
		...
		// July 2021 made this DPrint instead of regular, as it happens often
		Con_DPrintf ("bad connectionless packet from %s:\n%s\n"
...
int SV_BoundRate (qbool dl, int rate)
{
	...
	if (rate < 500)
		rate = 500;
	if (rate > 10000000) //STAL: increase from 100000 to 10000000 for better FTE support
		rate = 10000000;
...
void SV_Frame (double time1)
{
	...
	// OfN 2020 - Adjust move vars in real-time
	SV_SetMoveVars(); // is here the best place to set this? OfN
...
// OfN
void SV_SetStartInstanceTime(void)
{
	struct tm* startup_time_temp;
	time_t t;

	time(&t);
	startup_time_temp = localtime(&t);

	startup_time.day = startup_time_temp->tm_mday;
	startup_time.month = startup_time_temp->tm_mon;
	startup_time.year = startup_time_temp->tm_year + 1900;

	startup_time.second = startup_time_temp->tm_sec;
	startup_time.minute = startup_time_temp->tm_min;
	startup_time.hour = startup_time_temp->tm_hour;
}
//#define GIT_COMMIT "CPQWSV"
...
void SV_InitLocal (void)
{
	...
	static cvar_t qws_version = { "qws_version", "MVDSV " PQWSV_STRING, CVAR_ROM };
	//static cvar_t qws_buildnum = { "qws_buildnum", (GIT_COMMIT ? GIT_COMMIT : "unknown"), CVAR_ROM };
	//static cvar_t qws_buildnum = { "qws_buildnum", "PQWSV " VERSION_NUMBER, CVAR_ROM }; // OfN - Original above
	static cvar_t qws_buildnum = { "qws_buildnum", "MVDSV " PQWSV_STRING, CVAR_ROM }; // OfN - Original above
	...
	static cvar_t qwm_name = { "qwm_name", "Prozac" }; // TODO: Make this and the following set by the mod progs
	static cvar_t qwm_fullname = { "qwm_fullname", "Prozac CustomTF" };
	static cvar_t qwm_version = { "qwm_version", "3.2.OfN7" };
	static cvar_t qwm_buildnum = { "qwm_buildnum", "270+" };
	static cvar_t qwm_platform = { "qwm_platform", "All" };
	static cvar_t qwm_builddate = { "qwm_builddate", "28/06/2020+" };
	static cvar_t qwm_homepage = { "qwm_homepage", "http://quake.midioz.com" };
	...
	Cvar_Register(&pm_deadfix); // December 2021
...
//	Cvar_Register (&sv_highchars);
	...
	Cvar_Register (&sv_pinsfile); // OfN
	Cvar_Register(&sv_showheartbeats); // OfN
...
// <-- QW262
	...
	Info_SetValueForStarKey(svs.info, "*version", SERVER_NAME " " PQWSV_STRING, MAX_SERVERINFO_STRING);
	...
	pinmsg_cmdbuffer[0] = '\0';

	SV_SetStartInstanceTime(); // Set starting time for this server instance - OfN September 2020
	// OfN End
}
...
void SV_ExtractFromUserinfo (client_t *cl, qbool namechanged)
{
    ...
    // STAL: s2c packet dupes - from MVDSV 1.11
...
/* OfN
==================
SV_RetrieveClientVersion

Gets information from client infokeys, if any, and updates client & version fields
==================
*/

void SV_RetrieveClientVersion(client_t* theclient)
{
	if (!cltype_offset || !clversion_offset)
		return;
	else
	{
		float versionk;

		theclient->CL_Kind = CLTYPE_DEFAULT;
		theclient->CL_Version = 0.0;

		versionk = atof(Info_Get(&theclient->_userinfo_ctx_, "*ProzacQW"));

		if (versionk != 0)
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_PROZACQW;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = versionk;
			theclient->CL_Kind = CLTYPE_PROZACQW;
			theclient->CL_Version = versionk;
			return;
		}

		versionk = atof(Info_Get(&theclient->_userinfo_ctx_, "*FuhQuake"));

		if (versionk != 0)
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_FUHQUAKE;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = versionk;
			return;
		}

		versionk = atof(Info_Get(&theclient->_userinfo_ctx_, "*QWAMF"));

		if (versionk != 0)
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_AMFQUAKE;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = versionk;
			return;
		}

		versionk = atof(Info_Get(&theclient->_userinfo_ctx_, "*z_ver"));

		if (versionk != 0)
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_ZQUAKE;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = versionk;
			return;
		}

		versionk = atof(Info_Get(&theclient->_userinfo_ctx_, "*qf_version"));

		if (versionk != 0)
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_QUAKEFORGE;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = versionk;
			return;
		}

		if (!strncasecmp(Info_Get(&theclient->_userinfo_ctx_, "*ver"), "fte", 3)) // was 99999 - 2020 - Use 3?
		{
		...
		if (!strncasecmp(Info_Get(&theclient->_userinfo_ctx_, "*client"),"EZQuake",7))
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_EZQUAKE;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = (float)0;
			return;
		}

		if (!strncasecmp(Info_Get(&theclient->_userinfo_ctx_, "*client"),"FortressOne",11))
		{
			*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_FORTRESSONE;
			*(float*)((char*)&theclient->edict->v + clversion_offset) = (float)0;
			return;
		}

		versionk = atof(Info_Get(&theclient->_userinfo_ctx_, "*ver"));

		*(float*)((char*)&theclient->edict->v + cltype_offset) = (float)CLTYPE_DEFAULT;
		*(float*)((char*)&theclient->edict->v + clversion_offset) = versionk;
	}
}

//=================================================================
// OfN - Integration functions
//=================================================================

qbool SV_RetrieveMapIntegration(void)
{
	ddef_t	*d;
	int		*v;
	int		i, j;
	...
	for (e++; e < sv.num_edicts; e++)
	{
		...
		for (i = 1; i < progs->numfielddefs; i++)
		{
			...
			if (thisis_theone)
			{
				...
				{
					strcpy(CurrentIntegrationStr, (char*)PR_ValueString(d->type, (eval_t *)v));
					return true;
				}
			}
			else if (!strcmp(name, "classname"))
			{
				if (!strcmp((char*)PR_ValueString(d->type, (eval_t *)v), "world_integration"))
				{
					thisis_theone = true;
				}
			}
...
qbool SV_IssueIntegration(client_t* cl)
{
	if (cl->CL_Kind != CLTYPE_PROZACQW)
		return false;
	if (cl->CL_Version < MIN_PROZACVERSION_FORINTEGRATION)
		return false;

	char tmpstr[512 + 24];
	tmpstr[0] = 0;
	sprintf(tmpstr, "integration_issue %s\n", CurrentIntegrationStr);
	KK_stuffcmd(cl, tmpstr);

	return true;
}

void SV_BroadcastIntegrationToCompatibleClients(void)
{
	client_t	*client;
	int			j, numsent = 0;
	for (j = 0, client = svs.clients; j < MAX_CLIENTS; j++, client++)
	{
		...
		{
			if (SV_IssueIntegration(client))
				numsent++;
		}
	}
	if (numsent > 0)
		Sys_Printf("Sent integration to %d clients!\n", numsent);
	else
		Sys_Printf("No ProzacQW clients connected\n");
}
...
Prints pinned messages on a client, and increase the corresponding stat counters
==================
*/

void SV_SavePins(qbool silent);

void SV_DisplayPinmsgs(client_t* theclient)
{
	if (theclient->spectator)
		return;
