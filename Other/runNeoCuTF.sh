...
do
	...
	echo "Launching Neo Mode CustomTF 2026 (CustomTF 09 Fork) with PQWSV... (permanent batch loop)"
	...
	if [ "$1" == "public" ]; then
		...
		./mvdsv +gamedir fortress -port $PUBLIC_SERVER_PORT_ADDRESS +pausable 0 +exec server.cfg \
		        +setmaster $MASTER_SERVER_ADDRESSES +serverinfo neo 1 +sv_maxdownloadrate 10000000 -mem 96
	...
	# Use the following for a server that is to be used only for development, that will run on a different port.
		...
		./mvdsv +gamedir fortress -port $TEST_SERVER_PORT_ADDRESS +pausable 1 +password $TEST_SERVER_PASSWORD \
		        +exec server.cfg +serverinfo neo 1 +sv_maxdownloadrate 10000000 -mem 96
